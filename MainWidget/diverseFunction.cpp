...
void sudokuMain::mainTimerUp()
{
    ...
    lcdNumber->display(QString(strTime));      //显示
    m_iTimeSecond++;
    if(m_iTimeSecond>=60)
    {
        m_iTimeSecond = 0;
...
void sudokuMain::changeBackground(const QString & text)
{
    ...
    {
        //Change Setting
        if(text == QString(tr("紫色")))
        {
            m_cBackground = 'M';
            m_curBackground = QString(tr("紫色"));
        }
        else if(text == QString(tr("橘色")))
        {
            m_cBackground = 'X';
            m_curBackground = QString(tr("橘色"));
        }
        else if(text == QString(tr("黄色")))
        {
            m_cBackground = 'G';
            m_curBackground = QString(tr("黄色"));
        }
        else if(text == QString(tr("蓝色")))
        {
            m_cBackground = 'R';
            m_curBackground = QString(tr("蓝色"));
        }
...
void sudokuMain::enableIP()
{
    if(rB_single->isChecked())
    {
        ...
        label_p1->setText(QString(tr("已完成:")));
